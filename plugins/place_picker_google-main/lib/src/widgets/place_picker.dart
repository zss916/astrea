...
class PlacePicker extends StatefulWidget {
  ...
  /// Places net call debounce time in milli seconds
  ...
  final Widget placeTipWidget;

  final Widget placeListWidget;

  final Widget notFindPlaceWidget;
  ...
  const PlacePicker({
    ...
    required this.searchInputWidget,
    required this.placeTipWidget,
    required this.placeListWidget,
    required this.notFindPlaceWidget,
    this.mapsBaseUrl = 'https://maps.googleapis.com/maps/net/',
    this.mapsApiHeaders,
    this.mapsHttpClient,
    this.onMapCreated,
...
class PlacePickerState extends State<PlacePicker>
  ...
  /// List to populate nearby places from places net
  ...
  /// debounce timer to stop unwanted places net calls
  Timer? _debounce;
      ...
      children: [
        ...
        Container(
          ...
          child: Stack(
            ...
            children: [
              if (!widget.isInput)
                widget.placeTipWidget
              else
                Stack(
                  alignment: AlignmentDirectional.topCenter,
                  children: [
                    if (state == 1) widget.notFindPlaceWidget,
                    if (state == 2) widget.placeListWidget,
                    // if (state == 3)
  ...
  Widget _buildMapContent() {
    ...
    mapWidget =
        widget.tapGesturesEnabled ? mapWidget : IgnorePointer(child: mapWidget);
    return Stack(
      children: [
        Column(
          children: [
            ...
            child: Padding(
              ...
              child: CompositedTransformTarget(
                ...
                child: SearchInput(
                  ...
                  onSearchInput: autoCompleteSearch,
  ...
  Future<void> animateToLocation(LatLng latLng) async {
    ...
    //todo 经纬度LatLng=> 31.230416, 121.473701
    debugPrint("经纬度LatLng=> ${latLng.latitude}, ${latLng.longitude} ");
  ...
  Future<void> _reverseGeocodeLatLng(LatLng latLng) async {
    try {
        ...
        for (int resultIdx = 0;
          ...
          if (geocodingResultRaw.addressComponents != null &&
            ...
            for (int addressComponentsIdx = 0;
              ...
              /// `short` and `long` names from google net
      ...
      if (_geocodingResultList.isNotEmpty) {
        ...
        debugPrint(
            "获取到具体地址: name=>${_geocodingResult?.name},\ncountry=>${_geocodingResult?.country?.toJson()},\nformattedAddress=>${_geocodingResult?.formattedAddress},\nlatLng =>${_geocodingResult?.latLng} ");
      }
    } catch (e) {
      debugPrint(e.toString());
    }
  ...
  Future<void> autoCompleteSearch(String place) async {
    try {
      ...
      debugPrint('google place net response: ${predictions}');

      String s =
          "[{description: Shanghai, China, matched_substrings: [{length: 8, offset: 0}], place_id: ChIJMzz1sUBwsjURoWTDI5QSlQI, reference: ChIJMzz1sUBwsjURoWTDI5QSlQI, structured_formatting: {main_text: Shanghai, main_text_matched_substrings: [{length: 8, offset: 0}], secondary_text: China}, terms: [{offset: 0, value: Shanghai}, {offset: 10, value: China}], types: [political, geocode, locality]}]";

      //todo 显示名称是description

      if (status == PlacesAutocompleteStatus.zeroResults.status) {
        ///todo  Handle ZERO_RESULTS gracefully
      ...
      if (status != PlacesAutocompleteStatus.ok.status) {
        ///todo Log other non-OK statuses and clear suggestions
      ...
      ///todo Log error and clear suggestions as fallback
